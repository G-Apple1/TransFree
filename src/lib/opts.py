...
class opts(object):
  def __init__(self):
    ...
    self.parser.add_argument('task', default='ctdet',
                             help='ctdet | ddd | multi_pose | exdet| xu_ctdet')
    self.parser.add_argument('--dataset', default='pig_coco',
                             help='coco | kitti | coco_hp | pascal| pig_coco')
    self.parser.add_argument('--exp_id', default='coco_dla')
    ...
    self.parser.add_argument('--demo', default='/home/server/xcg/CenterNet/data/test/images',
                             help='path to image/ image folders/ video. '
                                  'or "webcam"')
    self.parser.add_argument('--load_model', default='/home/server/xcg/CenterNet/weights/model_final.pth',
    ...
    self.parser.add_argument('--arch', default='dla_34',
                             help='model architecture. Currently tested'
                                  'res_18 | res_101 | resdcn_18 | resdcn_101 | dlav0_34 | dla_34 | hourglass | swint')
    ...
    self.parser.add_argument('--lr_step', type=str, default='90,120,200',
    ...
    self.parser.add_argument('--val_intervals', type=int, default=10,
                             help='number of epochs to run validation.')#save best model
    ...
    '''soft_nms'''
    self.parser.add_argument('--soft_nms', action='store_true',
                             help='run soft_nms in testing.')
    self.parser.add_argument('--K', type=int, default=100,
                             help='max number of output objects.') 
    self.parser.add_argument('--not_prefetch_test', action='store_true',#default="--not_prefetch_test",
                             help='not use parallal data pre-processing.')
    self.parser.add_argument('--fix_res', action='store_true',
                             help='fix testing resolution or keep'
                                  'the original resolution')#test
    self.parser.add_argument('--keep_res', action='store_true',default="--keep_res",
                             help='keep the original resolution'
                                  ' during validation.')#val
    ...
    self.parser.add_argument('--no_color_aug', action='store_true',
                             help='not use the color augmenation '
                                  'from CornerNet')
    '''mosaic_mixup'''
    self.parser.add_argument('--mosaic_mixup', action='store_true',
                             help='data augment in train set')
    ...
    self.parser.add_argument('--scores_thresh', type=float, default=0.4,
                             help='threshold for extreme point heatmap.')
    self.parser.add_argument('--center_thresh', type=float, default=0.6,
  ...
  def init(self, args=''):
    default_dataset_info = {
      'ctdet': {'default_resolution': [960, 540], 'num_classes': 3,
                'mean': [0.408, 0.447, 0.470], 'std': [0.289, 0.274, 0.278],
                'dataset': 'pig_coco'},
      'xu_ctdet': {'default_resolution': [512, 512], 'num_classes': 3,
                  'mean': [0.408, 0.447, 0.470], 'std': [0.289, 0.274, 0.278],
                  'dataset': 'pig_coco'},
