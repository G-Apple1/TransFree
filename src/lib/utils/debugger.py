...
class Debugger(object):
  def __init__(self, ipynb=False, theme='black', 
    ...
    elif num_classes == 3 or dataset == 'pig_coco':
  ...
  def add_coco_bbox(self, bbox, cat, conf=1, show_txt=True, img_id='default', count=0):
    ...
    if self.theme == 'white':
      c = (255 - np.array(c)*10).tolist()
    txt = '{} {:.2f}'.format(self.names[cat], conf)
    font = cv2.FONT_HERSHEY_SIMPLEX
    cat_size = cv2.getTextSize(txt, font, 0.5, 2)[0]
    cv2.rectangle(
      self.imgs[img_id], (bbox[0], bbox[1]), (bbox[2], bbox[3]), c, 2)##c=color
    # cv2.putText(self.imgs[img_id], "COUNT: %d"%count, (30, 30),
    #             font, 1, (0, 255, 255), thickness=1, lineType=cv2.LINE_AA)
    # #if show_txt:
    #   # cv2.rectangle(self.imgs[img_id],
    #   #               (bbox[0], bbox[1] - cat_size[1] - 2),
    #   #               (bbox[0] + cat_size[0], bbox[1] - 2), c, 4)
