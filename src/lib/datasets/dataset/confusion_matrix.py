...
class ConfusionMatrix:
    ...
    def process_batch(self, detections, labels):#
        ...
        if x[0].shape[0]:
            ...
            matches = torch.cat((torch.stack(x, 1).to(torch.float64), iou[x[0], x[1]][:, None]), 1).cpu().numpy()
            if x[0].shape[0] > 1:
    ...
    def plot(self, normalize=True, save_dir='', names=(), rec_or_pred=0):#0=recall, 1=pred
        try:
            ...
            array = self.matrix.astype(np.int)  #+ array
            ...
            with warnings.catch_warnings():
                ...
                sn.heatmap(array, annot=self.nc < 30, annot_kws={"size": 12}, cmap='Blues', fmt='d', square=True,
                           xticklabels=names + ['background FP'] if labels else "auto",
                           yticklabels=names + ['background FN'] if labels else "auto").set_facecolor((1, 1, 1))#
            fig.axes[0].set_xlabel('True')
            fig.axes[0].set_ylabel('Predicted')
            if rec_or_pred == 0:
                ALL = sum([self.matrix[k, k] for k in range(self.nc)]) / sum(self.matrix.sum(rec_or_pred)[:-1])
                fig.suptitle("Recall (%.2f)"%(ALL*100))
            else:
                ALL = sum([self.matrix[k, k] for k in range(self.nc)]) / sum(self.matrix.sum(rec_or_pred)[:-1])
                fig.suptitle("Precision (%.2f)"%(ALL*100))
            #plt.show()
...
def process_batch(detections, labels, iouv):
    ...
    if x[0].shape[0]:
        matches = torch.cat((torch.stack(x, 1).to(torch.float64), iou[x[0], x[1]][:, None]), 1).cpu().numpy()  # [label, detection, iou]
        if x[0].shape[0] > 1:
            matches = matches[matches[:, 2].argsort()[::-1]]
