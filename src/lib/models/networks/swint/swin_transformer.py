...
from detectron2.modeling.backbone.fpn import FPN, LastLevelMaxPool, LastLevelP6P7, P4LevelMaxPool
...
#         if in_features == 768 or self.in_features==384 or self.in_features==192:
#             self.fc1 = nn.Linear(in_features, hidden_features)
#             self.act = act_layer()
#             self.fc2 = nn.Linear(hidden_features, out_features)
#             self.drop = nn.Dropout(drop)
...
#             self.drop = nn.Dropout(drop)
#         self.fm_sizes = {"96_h":136,"96_w":248,"192_h":68,"192_w":124,"384_h":34,"384_w":62}
#
#     def forward(self, x):
#         if self.in_features == 768 or self.in_features==384  or self.in_features==192:
#             # print("no ffn", self.in_features, x.shape)
#             x = self.fc1(x)
#             x = self.act(x)
#             x = self.drop(x)
#             x = self.fc2(x)
#             x = self.drop(x)
#         else:
#             # print("+++ffn",self.in_features,x.shape)
#             B,N,C = x.shape
#             x = x.reshape(B, self.fm_sizes[f"{C}_h"], self.fm_sizes[f"{C}_w"], C).permute(0,3,1,2)
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        if self.shuffle:
            '''shuffle module'''
            qkv = self.qkv(x).reshape(B_, 7, 7, C * 3).permute(0, 3, 1, 2)
...
#     def forward(self, x, H, W):
#         B, new_HW, C = x.shape
#         assert new_HW == H * W, "input feature has wrong size"
...
#         # x = x.transpose(1, 2).view(-1, 96, H // 4, W // 4)
...
class SwinTransformer(Backbone):
    ...
    def forward(self, x):
        ...
        # print(outs_fm)
        # print("---------------")
        # plot_fm(outs_fm['stage2'],"stage2")
        # io.savemat("/home/server/xcg/SwinT_detectron2/fm_task/train_fm_all/"+
        #            self.img_name[:-4]+".mat", outs_fm)
...
def build_swint_backbone(cfg, input_shape):
    ...
    return SwinTransformer(
        ...
        img_name=cfg.CURRENT_IMAGE_NAME
    )
...
def build_swint_fpn_backbone(cfg, input_shape: ShapeSpec):###
    ...
    print(cfg.CURRENT_IMAGE_NAME)
    '''fpn's top_block=None/LastLevelMaxPool()/P4LevelMaxPool()
    None: p2\p3\p4
    last: p2\p3\p4\p5
    p4: p2\p3\p4\p5
    LastLevelP6: p2\p3\p4\p5\p6
    '''
    backbone = FPN(
        bottom_up=bottom_up,
        ...
        top_block=None,#LastLevelMaxPool(),#p4,#
