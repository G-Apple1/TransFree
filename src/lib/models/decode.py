...
def _nms(heat, kernel=3):
    ...
    #import numpy as np
    #plt.figure()
    #dots=np.where(heat[0][0].cpu().numpy() > 0.1)
    #list_dots = list(dots)

    # with open("list_dots_0.05.txt", 'w') as f:
    #     for i in range(len(list_dots)):
    #         s = str(list_dots[i]).replace('[', '').replace(']', '')  # 去除[],这两行按数据不同，可以选择
    #         s = s.replace("'", '').replace(',', '') + '\n'  # 去除单引号，逗号，每行末尾追加换行符
    #         print(s)
    #         f.write(s)
    #
    ...
    # np.savetxt('list_dots_0.1.txt', dots, fmt='%.1f')
...
def _topk(scores, K=40):
    ...
    # import numpy as np
    # plt.figure()
    # dots=np.where(scores[0][0].cpu().numpy() > 0.5)
    # plt.scatter(dots[0],dots[1])
    # plt.show()
    topk_scores, topk_inds = torch.topk(scores.view(batch, cat, -1), K)
